...
public class APManager {
    ...
    public interface OnApListener {
        void onApFailure(int failureCode, @Nullable Exception e);
        void onApSuccess(@NonNull String ssid, @NonNull String password);
    }
    ...
    public void turnOnHotspot(Context context, final OnApListener onResult/*, final OnFailureListener onFailureListener*/) {
...
//        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            if (utils.checkLocationPermission(context) && providerEnabled && !isWifiApEnabled()) {
                    ...
                    wifiManager.startLocalOnlyHotspot(new WifiManager.LocalOnlyHotspotCallback() {
                        ...
                        public void onStarted(WifiManager.LocalOnlyHotspotReservation reservation) {
                            ...
                            try {
                                ...
                                onResult.onApSuccess(ssid, password);
                            } catch (Exception e) {
                                e.printStackTrace();
                                onResult.onApFailure(ERROR_UNKNOWN,e);
                            }
                        }

                        public void onFailed(int reason) {
                            super.onFailed(reason);
                            onResult.onApFailure(reason == ERROR_TETHERING_DISALLOWED ? ERROR_DISABLE_HOTSPOT : ERROR_UNKNOWN,null);
                        }
                ...
                onResult.onApFailure(ERROR_GPS_PROVIDER_DISABLED,null);
            } else if (isWifiApEnabled()) {
                onResult.onApFailure(ERROR_DISABLE_HOTSPOT,null);
            } else {
                onResult.onApFailure(ERROR_LOCATION_PERMISSION_DENIED,null);
            }
        } else {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                if (!utils.checkWriteSettingPermission(context)) {
                    onResult.onApFailure(ERROR_WRITE_SETTINGS_PERMISSION_REQUIRED,null);
                    return;
                }
            }
            try {
                ...
                e.printStackTrace();
                onResult.onApFailure(ERROR_LOCATION_PERMISSION_DENIED,e);
            }
        }
    }
